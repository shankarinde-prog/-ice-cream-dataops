...
def get_time_series_for_site(client: CogniteClient, site):
    ...
    # Retrieve the root node for the site
    ...
    # Standard subtree traversal using CogniteAsset TypedNode and sub_tree_root.path
    if sub_tree_root.path:
        ...
        # Fallback if path is not yet materialized on the root node
    ...
    for batch in batcher(value_list, 20):
        ...
        time_series.extend(res)

    if not time_series:
        print(f"No CogniteTimeSeries found for {site}")
